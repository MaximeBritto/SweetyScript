...
local function updatePlatformPromptText(platform, player)
    ...
    if not isUnlocked then
        ...
        if idx and idx > unlockedCount + 1 then
            proximityPrompt.ActionText = "Verrouillé"
            proximityPrompt.ObjectText = "Débloquez d'abord Plateforme " .. (unlockedCount + 1)
        else
            local cost = getUnlockCostForIndex(idx or (unlockedCount + 1))
            proximityPrompt.ActionText = "Débloquer"
            proximityPrompt.ObjectText = "Plateforme " .. (idx or "?") .. " (" .. cost .. "$)"
        end
    elseif isOccupied then
        -- Il y a déjà un bonbon sur la plateforme
        if hasCandy then
            proximityPrompt.ActionText = "Remplacer"
            proximityPrompt.ObjectText = "Bonbon sur Plateforme"
        else
            proximityPrompt.ActionText = "Retirer"
            proximityPrompt.ObjectText = "Bonbon de la Plateforme"
        end
    else
        -- Plateforme vide
        if hasCandy then
            proximityPrompt.ActionText = "Placer"
            proximityPrompt.ObjectText = "Bonbon sur Plateforme"
        else
            proximityPrompt.ActionText = "Placer"
            proximityPrompt.ObjectText = "Bonbon (Équipez d'abord)"
        end
    end
end
